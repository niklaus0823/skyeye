...
class CLI {
    ...
    private _waitForResponse() {
        ...
        setTimeout(async () => {
            ...
            if (!lock) {
                const res = await cache.hGet(CACHE_SERVER_STAT, AGENT_ID);
                ...
                console.log(
                    ...
                    `| ${CommonTools.padding('CPU', 10, ' ', true)}`,
                    `| ${CommonTools.padding('CTIME', 10, ' ', true)}`
                );
                console.log(
                    `| ${CommonTools.padding('', 20, '-', true)}`,
                    `| ${CommonTools.padding('', 10, '-', true)}`,
                ...
                console.log(
                    `| ${CommonTools.padding(AGENT_ID, 20, ' ', true)}`,
                    `| ${CommonTools.padding(res.cpu, 10, ' ', true)}`,
                    `| ${CommonTools.padding(res.ctime, 10, ' ', true)}`
                );
                console.log(
                    `| ${CommonTools.padding('', 20, '-', true)}`,
                    `| ${CommonTools.padding('', 10, '-', true)}`,
