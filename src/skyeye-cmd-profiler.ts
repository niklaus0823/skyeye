...
import {CACHE_AGENT_LIST, CACHE_AGENT_LOCK, CACHE_CPU_PROFILER} from './model/agent/AgentManager';
...
const TIMEOUT = (program as any).timeout === undefined ? 1000 : (program as any).timeout; // 1 min
...
class CLI {
    ...
    private async _send() {
        ...
        ws.on('open', () => {
            ...
            ws.send(PacketModel.create(API_TYPE.EXEC_CPU_PROFILER, {id: AGENT_ID, timeout: TIMEOUT}).format());
    ...
    private _waitForResponse() {
        ...
        setTimeout(async () => {
            ...
            if (!lock) {
                const res = await cache.hGet(CACHE_CPU_PROFILER, AGENT_ID);
                console.log(res);
                ...
                console.log(
                    ...
                    `| ${CommonTools.padding('', 10, '-', true)}`,
                    `| ${CommonTools.padding('', 30, '-', true)}`
                );
                console.log(
                    `| ${CommonTools.padding('funName', 20, ' ', true)}`,
                    `| ${CommonTools.padding('execTime:', 10, ' ', true)}`,
                    `| ${CommonTools.padding('percentage', 10, ' ', true)}`,
                    `| ${CommonTools.padding('url', 30, ' ', true)}`,
                );
                console.log(
                    `| ${CommonTools.padding('', 20, '-', true)}`,
                    `| ${CommonTools.padding('', 10, '-', true)}`,
                ...
                if (res.hasOwnProperty('topExecutingFunctions')) {
                    for (let item of res.topExecutingFunctions) {
                        console.log(
                            `| ${CommonTools.padding(item.funcName, 20, ' ', true)}`,
                            `| ${CommonTools.padding(item.execTime, 10, ' ', true)}`,
                            `| ${CommonTools.padding(item.percentage, 10, ' ', true)}`,
                            `| ${CommonTools.padding(item.url, 30, ' ', true)}`
                        );
                    }
                }
                console.log(
                ...
                // TOP EXECUTING FUNCTIONS
                console.log(
                    `| TIMEOUT FUNCTIONS, timeout(${TIMEOUT}ms)`,
                );
                console.log(
                    `| ${CommonTools.padding('', 20, '-', true)}`,
                    `| ${CommonTools.padding('', 10, '-', true)}`,
